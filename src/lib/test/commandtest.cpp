...
CPPUNIT_TEST_SUITE_NAMED_REGISTRATION(CommandTest, CommandTest::SuiteName());
...
void CommandTest::Constructor() {
	...
	// valid command name
	CPPUNIT_ASSERT_NO_THROW(command = shared_ptr<Command>(new Command("test")));

	// invalid command name
	CPPUNIT_ASSERT_THROW((command=shared_ptr<Command>(new Command("¶¡£ðøæ·"))),
															invalid_argument);
}
...
void CommandTest::ParseArguments() {
	...
	command = shared_ptr<Command>(new Command("cmd"));
	argument_list.clear();
	argument_list.push_back(shared_ptr<Argument>(new WordArgument));
	command->AddArguments(argument_list);
	...
	CPPUNIT_ASSERT_EQUAL(string("word_argument"), command->GetWord(1));
	...
	CPPUNIT_ASSERT_NO_THROW(command->ParseArguments(command_line));
	CPPUNIT_ASSERT_EQUAL(string("string argument"), command->GetString(1));
	...
	CPPUNIT_ASSERT_NO_THROW(command->ParseArguments(command_line));
	CPPUNIT_ASSERT_EQUAL(string("word_argument"), command->GetWord(1));
	CPPUNIT_ASSERT_EQUAL(string("string argument"), command->GetString(2));
	...
	CPPUNIT_ASSERT_NO_THROW(command->ParseArguments(command_line));
	CPPUNIT_ASSERT_EQUAL(uint32_t(123), command->GetNumber(1));
	CPPUNIT_ASSERT_EQUAL(string("word_argument"), command->GetWord(2));
	...
	CPPUNIT_ASSERT_EQUAL(string("word_argument"),  command->GetWord(2));
	CPPUNIT_ASSERT_EQUAL(string("word_argument2"), mirror_command.GetWord(2));
	...
	command_line = "cmd 123 word_argument";
	CPPUNIT_ASSERT_THROW(command->GetWord(2), logic_error);

	// try to get argument value from an argument of the wrong type
	CPPUNIT_ASSERT_NO_THROW(command->ParseArguments(command_line));
	CPPUNIT_ASSERT_THROW(command->GetWord(1), logic_error);

	// try to get argument value from a nonexistent argument
	CPPUNIT_ASSERT_THROW(command->GetWord(10), logic_error);
}
